                ...
                <tbody>
                  @foreach ($data as $c)
                    <tr id="{{ $c->id }}">
                      ...
                      <td>{{ $c->name }}</td>
                      <td>{{substr($c->description, 0, 22).'...'}}</td>
                      <td>{{ $c->author }}</td>
                      <td>{{ $c->price }}</td>
                      <td>{{ $c->inventory }}</td>
                      <td>
                        @foreach ($dataCategory as $item)
                        @if (($c->id_category)==($item->id))
                          {{ $item->name }}
                        @endif
                        @endforeach
                      </td>
                      <td>
                        ...
                        @if (($c->id_publisher)==($item->id))
                          {{ $item->name }}
                        @endif
                        @endforeach
                      </td>
                      <td>
                        @foreach ($dataLanguge as $item)
                      ...
                      <td class="delete"><a href="/admin/delete-book/{{ $c->id }}"
